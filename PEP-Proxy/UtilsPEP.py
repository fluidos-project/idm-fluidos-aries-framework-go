...
import json
import API.UtilsNGSIv1,API.UtilsNGSIv2,API.UtilsNGSILDv1,API.UtilsGenericAPI
import sys
    ...
    try:
        ...
        if (len(rPAE) > 0 or len(rPAE)):
            ...
            if ((APIVersion.upper() == "NGSIv1".upper() or  
                ...
                {'key': 'value'}
                {"key": "val'ue"}
                {'key': 'val"ue'} --> Double quote will be escaped in the value contain.
                '''
                try:
                    bodyInProcess = json.loads(body.decode('utf8').replace("'", '"'))
                except:
                    try:
                        bodyInProcess = json.loads(body.decode('utf8'))
                    except:
                        bodyInProcess = json.loads(body.decode('utf8').replace('"', '\\"').replace("'", '"'))
