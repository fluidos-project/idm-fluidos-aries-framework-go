...
def processBody(method,uri,body,sPAE,rPAE,noEncryptedKeys):
    ...
    try:
        ...
        if ((method.upper()=="POST".upper() or method.upper()=="PATCH".upper())
                #and uri.upper()=="/v2/entities".upper())
                ):
...
def processCypherAttr(body,sPAE,rPAE,noEncryptedKeys):
    ...
    try:
        ...
        if  ("metadata" in body):
            if  ("encrypt_cpabe" in body["metadata"]):
                if  ("type" in body["metadata"]["encrypt_cpabe"] and "value" in body["metadata"]["encrypt_cpabe"]):
                    if  (body["metadata"]["encrypt_cpabe"]["type"]=="policy"):
                        if(str(body["metadata"]["encrypt_cpabe"]["value"]).strip()!=""):
        ...
        for m in range(len(encryptAttributes)):
            try:
                ...
                if(body[encryptAttributes[m]]["metadata"]["encrypt_cpabe"]["type"]=="policy"): #CPABE ENCRYPTATION
                    if(str(body[encryptAttributes[m]]["metadata"]["encrypt_cpabe"]["value"]).strip()!=""): #CPABE ENCRYPTATION
