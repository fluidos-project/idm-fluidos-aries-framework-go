...
try:
    node_verifier_host = str(os.getenv('node_verifier_host'))
except Exception as e:
    logging.error(e)
    node_verifier_host = "localhost"

if (str(node_verifier_host).upper() == "None".upper()) :
    node_verifier_host = "localhost"

node_verifier_port = 9082
try:
    node_verifier_port = int(os.getenv('node_verifier_port'))
except Exception as e:
    logging.error(e)
    node_verifier_port = 9082
    ...
    try:
        ...
        if(target_protocol.upper() == "http".upper() or target_protocol.upper() == "https".upper()):
                ...
                if(node_verifier_protocol.upper() == "http".upper()):
                    conn = http.client.HTTPConnection(node_verifier_host, node_verifier_port)
                else:
                    #gcontext = ssl.SSLContext()
                    #conn = http.client.HTTPSConnection(node_verifier_host,node_verifier_port,
                    #                            context=gcontext)
                    # Secure Client SSL Context
                    #gcontext = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
                    #gcontext.check_hostname = True
            ...
            for key in headers:
                ...
                if(key.upper()=="x-auth-token".upper()):
                    ...
                    if (isValidJWT):
                        ...
                        if (decoded["method"].upper() == method.upper()):
                            ...
                            if (currentTimestamp >= decoded["exp"]):
                                ...
                                for i in range(len(pep_device_list)):
                                    ...
                                    pattern1 = re.compile("^" + decoded["resource"] + "$")
                                    matcher1 = pattern1.match(pep_device_list[i]+uri)
