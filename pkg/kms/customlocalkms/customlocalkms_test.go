...
func TestCustomLocalKMS_Success(t *testing.T) {
	// create a real (not mocked) master key and secret lock to test the KMS end to end
	sl := createMasterKeyAndSecretLock(t)
	...
	psmsKeyOpts := kms.WithAttrs([]string{"4"})
	...
	for _, v := range keyTemplates {
		...
		if v == kms.BLS12381G1Type { //TODO UMU (test) Test the case where no key opts are given (i.e., error is thrown or default key is created)
			keyID, newKeyHandle, e := kmsService.Create(v, psmsKeyOpts)
			require.NoError(t, e, "failed on template %v", v)
			require.NotEmpty(t, newKeyHandle)
			require.NotEmpty(t, keyID)
			ks, ok := kmsService.mockStore.Retrieve(keyID)
			require.True(t, ok)
			...
			require.Equal(t, kt, kms.BLS12381G1Type)
...
func TestCustomLocalKMS_getKeyTemplate(t *testing.T) {
