...
func createLocalKeyManager(user, passphrase, masterLockCipher string,
	masterLocker secretlock.Service, storeProvider kms.Store) (*customlocalkms.CustomLocalKMS, error) {
	var err error
	if passphrase != "" {
	...
	return customlocalkms.New(localKeyURIPrefix+user, &kmsProvider{
...
func newKMSSigner(authToken string, c crypto.Crypto, opts *ProofOptions) (*kmssigner.KMSSigner, error) {
	...
	return &kmssigner.KMSSigner{
		...
		Crypto:    c,
		MultiMsg:  (opts.ProofType == BbsBlsSignature2020 || opts.ProofType == PsmsBlsSignature2022),
	}, nil
}
...
func importKeyBase58(auth string, key *keyContent) error {
	...
	case Bls12381G1Key2022:
		blsKey, err := psms12381g1pub.UnmarshalPrivateKey(base58.Decode(key.PrivateKeyBase58))
		if err != nil {
			return fmt.Errorf("failed to unmarshal %s private key : %w", kms.BLS12381G2Type, err)
		}

		_, _, err = keyManager.ImportPrivateKey(blsKey, kms.BLS12381G1Type, kms.WithKeyID(getKID(key.ID)))
		if err != nil {
			return fmt.Errorf("failed to import Ed25519Signature2018 key : %w", err)
		}
	default:
