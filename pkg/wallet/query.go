...
//TODO UMU ZK Check queryByExample and presentationDefinition
...
func (cm *credentialMatcher) MatchFrame(credential *verifiable.Credential) bool {
	...
	// also check if VC has bbs or psms signature
	for _, proof := range credential.Proofs {
		if proof["type"] == PsmsBlsSignature2022 || proof["type"] == BbsBlsSignature2020 {
			return true
		}
	}
...
func queryByFrame(vcs []*verifiable.Credential, publicKeyFetcher verifiable.PublicKeyFetcher, loader ld.DocumentLoader,
	...
	for _, vc := range vcs {
		for _, definition := range definitions {
			...
			// match frame
			if hasPSMS(vc) { //TODO UMU (issue) Nil Nonce for a presentation? That's not good
				psmsVC, err := vc.GeneratePSMSSelectiveDisclosure(definition.Frame, nil,
