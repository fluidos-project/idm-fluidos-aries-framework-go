...
const (
	...
	// PsmsBlsSignature2020 PSMS signature suite.
...
func (c *Wallet) Derive(authToken string, credential CredentialToDerive, options *DeriveOptions) (*verifiable.Credential, error) { //nolint: lll
	...
	var derived *verifiable.Credential
	if hasPSMS(vc) {
		derived, err = vc.GeneratePSMSSelectiveDisclosure(options.Frame, []byte(options.Nonce),
...
func (c *Wallet) CreateKeyPair(authToken string, keyType kms.KeyType, kOpts ...kms.KeyOpts) (*KeyPair, error) {
	session, err := sessionManager().getSession(authToken)
	if err != nil {
		return nil, err
	}

	kid, pubBytes, err := session.KeyManager.CreateAndExportPubKeyBytes(keyType, kOpts...)
	if err != nil {
		return nil, err
	}
...
func (c *Wallet) addLinkedDataProof(authToken string, p provable, opts *ProofOptions,
	...
	case PsmsBlsSignature2022:
		addContext(p, psmsContext)

		signatureSuite = psmsblssignature2022.New(suite.WithSigner(s))
	default:
	...
	} // XXX UMU (verifiable) If adding fields to proof check
...
func hasPSMS(vc *verifiable.Credential) bool {
	return hasProofWithType(vc, "PsmsBlsSignature2022")
}
