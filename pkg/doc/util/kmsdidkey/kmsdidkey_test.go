...
func TestBuildDIDKeyByKMSKeyType(t *testing.T) {
	...
	_, psmsKey, err := k.CreateAndExportPubKeyBytes(kms.BLS12381G1Type, kms.WithAttrs([]string{"4"}))
	require.NoError(t, err)
	...
	tests := []struct {
		...
		{
			name:     "test BLS12381G1 key",
			keyBytes: psmsKey,
			keyType:  kms.BLS12381G1Type,
		},
	}
...
func newKMS(t *testing.T, store storage.Provider) kms.KeyManager {
	...
	customLocalKMS, err := customlocalkms.New("local-lock://primary/test/", kmsProv)
	require.NoError(t, err)
