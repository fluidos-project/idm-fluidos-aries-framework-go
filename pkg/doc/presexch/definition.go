...
func frameCreds(frame map[string]interface{}, creds []*verifiable.Credential,
	...
	for _, credential := range creds {
		...
		} else if hasPSMS(credential) {
			psmsVC, err := credential.GeneratePSMSSelectiveDisclosure(frame, nil, opts...)
			if err != nil {
				return nil, err
			}

			result = append(result, psmsVC)
		}
	}
...
func createNewCredential(constraints *Constraints, src, limitedCred []byte,
	...
	var (
		...
		PSMSSupport         = hasPSMS(credential)
	...
	for _, f := range constraints.Fields {
		...
		for _, path := range jPaths {
			...
			if constraints.LimitDisclosure.isRequired() && (BBSSupport || PSMSSupport) {
	...
	if !constraints.LimitDisclosure.isRequired() || !(BBSSupport || PSMSSupport) || modifiedByPredicate {
	...
	if PSMSSupport {
		return credential.GeneratePSMSSelectiveDisclosure(doc, []byte(uuid.New().String()), opts...)
	}
