...
func TestPresentationDefinition_CreateVP(t *testing.T) {
	...
	t.Run("Limit disclosure PSMS", func(t *testing.T) {
		required := Required
			...
			Subject: verifiable.Subject{
				...
				CustomFields: map[string]interface{}{
					...
					"degree": map[string]interface{}{
						//	"degree":       "MIT", XXX UMU (issue/verifiable) See suite, composite attributes do not work properly with selective disclosure/framing
		...
		nattr := 3
		publicKey, privateKey, err := psms12381g1pub.New().GenerateKeyPair(seed, nattr)
		require.NoError(t, err)

		srcPublicKey, err := publicKey.Marshal()
		require.NoError(t, err)

		signer, err := newPSMSSigner(privateKey)
		require.NoError(t, err)

		require.NoError(t, vc.AddLinkedDataProof(&verifiable.LinkedDataProofContext{
			SignatureType:           "PsmsBlsSignature2022",
			SignatureRepresentation: verifiable.SignatureProofValue,
			Suite:                   psmsblssignature2022.New(suite.WithSigner(signer)),
		...
		vp, err := pd.CreateVP([]*verifiable.Credential{vc}, lddl,
			...
			verifiable.WithPublicKeyFetcher(verifiable.SingleKey(srcPublicKey, "Bls12381G1Key2022")),
		)
		require.NoError(t, err)
		require.NotNil(t, vp)
		require.Equal(t, 1, len(vp.Credentials()))
	...
	t.Run("Predicate and limit disclosure PSMS (no proof)", func(t *testing.T) {
		required := Required
