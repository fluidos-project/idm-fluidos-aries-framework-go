...
func (j *JWK) PublicKeyBytes() ([]byte, error) { //nolint:gocyclo
	if j.isBLS12381G1() {
		switch psmsKey := j.Key.(type) {
		case *psms12381g1pub.PrivateKey:
			return psmsKey.PublicKey().Marshal()
		case *psms12381g1pub.PublicKey:
			return psmsKey.Marshal()
		}
	}
...
func (j *JWK) UnmarshalJSON(jwkBytes []byte) error {
	...
	if isSecp256k1(key.Alg, key.Kty, key.Crv) {
		...
		*j = *jwk
	} else if isBLS12381G1(key.Kty, key.Crv) {
		jwk, err := unmarshalBLS12381G1(&key)
		if err != nil {
			return fmt.Errorf("unable to read BBS+ JWE: %w", err)
		}
...
func (j *JWK) KeyType() (kms.KeyType, error) {
	...
	case *psms12381g1pub.PrivateKey, *psms12381g1pub.PublicKey:
		return kms.BLS12381G1Type, nil
	case *ecdsa.PublicKey:
...
func (j *JWK) isBLS12381G1() bool {
	switch j.Key.(type) {
	case *psms12381g1pub.PublicKey, *psms12381g1pub.PrivateKey:
		return true
	default:
		return false
	}
...
func isBLS12381G1(kty, crv string) bool {
	return strings.EqualFold(kty, ecKty) && strings.EqualFold(crv, bls12381G1Crv)
}
...
func unmarshalBLS12381G1(jwk *jsonWebKey) (*JWK, error) {
	if jwk.X == nil {
		return nil, ErrInvalidKey
	}
	...
	if jwk.D != nil {
		key, err = psms12381g1pub.UnmarshalPrivateKey(jwk.D.data)
		if err != nil {
			return nil, fmt.Errorf("jwk invalid private key unmarshal: %w", err)
		}
	} else {
		key, err = psms12381g1pub.UnmarshalPublicKey(jwk.X.data)
		if err != nil {
			return nil, fmt.Errorf("jwk invalid public key unmarshal: %w", err)
		}
	}
...
func marshalBLS12381G1(jwk *JWK) ([]byte, error) {
	...
	case *psms12381g1pub.PrivateKey:
		...
		raw = jsonWebKey{
			Kty: ecKty,
			Crv: bls12381G2Crv,
			X:   newVariableSizeBuffer(mPubKey),
			D:   newVariableSizeBuffer(mPrivKey),
		}
	default:
		return nil, errors.New("marshalBLS12381G2: invalid key")
	}
...
func newVariableSizeBuffer(data []byte) *byteBuffer {
	return &byteBuffer{
		data: data,
	}
}
