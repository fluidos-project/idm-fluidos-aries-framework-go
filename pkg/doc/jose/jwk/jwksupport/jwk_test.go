...
func TestDecodeJWK(t *testing.T) {
	t.Run("Test decode public key success", func(t *testing.T) {
		tests := []struct {
			...
			{
				name: "get public key bytes PSMS JWK",
				//nolint:lll
				jwkJSON: `{
							"kty": "EC",
							"use": "enc",
							"crv": "BLS12381_G1",
							"kid": "sample@sample.id",
							"x": "AAMDBBlJysWwBIMId4bynPl2HhE4-W_uNYNFaem1vlPtmZDyi_qrLpNSype9qjeofxGiPxBB9t1ylXKmcLouJ8R` +
					`NTHs_wWO-gfCXmhDurByoxmj1fqa0_34BmK6Ln2Tm_hBGxAQQZ8EXljvuXJYZJtU_d36QetWgKZMV3Leuns0wwD7NEP6UFkZV7kb` +
					`sSaeR5PctRe4PLJ2Tx8_le35VT9WJRxZ3MzDRWDSyh7KiEYi2kRLC178xdCkZsCBezwJtE4xQfLIEDvhChUQGTZHbTIwNQJ-iwRB` +
					`B7ejKYLMPMKRvuxFZZz5WprFxuZYEwCUuMipKkSyrA0UwVDk9BSRVrWgdrYq2njLxqB8t6NJGOfWJe7EsXMnFQcOodx7KA5VcgwE` +
					`ln7iLBA94W76j3sNeusEF6ov7kyath7o2TbQfQoxviRcZBcKSisOON-dkDW-AGLwrYrealg1gaoVJP1_YHqoG2LTmuvYLAw1836w` +
					`3sGm27zKIvZvlPwDbpaE5f501NGDlhZagFAQSfL-ExaVcWPaiNKSJLqhH0XpfP2waemPPFIlXeMz8vyEJxntMS9hx4lMxCvUt4zs` +
					`Bh3wEtC5JlWtkI3x6H0qrQBQKbBDW8Bf48_ewPJKpuJORSjHeP9E0f2iIT3GXAGcEDGsRHpR4wPoF08CvPFdCqJEZO6_7n9BOljG` +
					`z9QCJlXGKWD_1AR8NQIFaZv9fnk_cBbcniocSO6v_Cxl6pr5bdmDhNV-x3ND7HwIXeBsW2A13Nl4x9xpoQooEAAwaND2F"
						}`,
			},
			{
		...
		for _, test := range tests {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				case "get public key bytes EC P-521 JWK":
					...
					require.Equal(t, "AZzRvW8NBytGNbF3dyNOMHB0DHCOzGp8oYBv_ZCyJbQUUnq-TYX7j8-PlKe9Ce5acxZzrcUKVtJ4I8JgI5x9oXIW",
						base64.RawURLEncoding.EncodeToString(ecKey.Y.Bytes()))
				case "get public key bytes PSMS JWK": //TODO UMU (test) Complete rest of tests
					jwkKey2, err := JWKFromKey(jwkKey.Key)
					require.NoError(t, err)
					require.NotNil(t, jwkKey2)
					require.Equal(t, bls12381G1Crv, jwkKey2.Crv)
					psmsPubKey, ok := jwkKey2.Key.(*psms12381g1pub.PublicKey)
					require.True(t, ok)
					_, err = psmsPubKey.Marshal()
					require.NoError(t, err)
					require.Equal(t, ecKty, jwkKey2.Kty)

					newJWK, err := PubKeyBytesToJWK(pkBytes, kms.BLS12381G1Type)
					require.NoError(t, err)
					require.NotNil(t, newJWK)
					require.Equal(t, bls12381G1Crv, newJWK.Crv)
					psmsPubKey, ok = newJWK.Key.(*psms12381g1pub.PublicKey)
					require.True(t, ok)
					_, err = psmsPubKey.Marshal()
					require.NoError(t, err)
					require.Equal(t, ecKty, newJWK.Kty)
