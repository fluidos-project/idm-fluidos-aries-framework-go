...
// *bbs12381g2pub.PublicKey., *psms12381g1pub.PrivateKey, *psms12381g1pub.PublicKey
...
func PubKeyBytesToJWK(bytes []byte, keyType kms.KeyType) (*jwk.JWK, error) { // nolint:gocyclo
	...
	case kms.BLS12381G1Type:
		psmsKey, err := psms12381g1pub.UnmarshalPublicKey(bytes)
		if err != nil {
			return nil, err
		}
		return JWKFromKey(psmsKey)
...
func PublicKeyFromJWK(jwkKey *jwk.JWK) (*cryptoapi.PublicKey, error) {
	if jwkKey != nil {
		...
		case *psms12381g1pub.PublicKey:
			psmsKey, _ := key.Marshal() //nolint:errcheck // psms marshal public key does not return any error
			pubKey.X = psmsKey
		case *psms12381g1pub.PrivateKey:
			psmsKey, _ := key.PublicKey().Marshal() //nolint:errcheck // psms marshal public key does not return any error
			pubKey.X = psmsKey
