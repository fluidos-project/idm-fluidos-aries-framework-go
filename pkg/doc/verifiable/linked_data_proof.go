...
func (k *keyResolverAdapter) Resolve(id string) (*verifier.PublicKey, error) {
	...
	idSplit := strings.Split(id, "#") //XXX UMU (issue) KeyID is assumed to always be issuer#keyID, maybe should depend on VDR/DidMethod
...
func mapContext(context *LinkedDataProofContext) *signer.Context {
	// XXX UMU (verifiable) Maybe useful if we need extra fields to proof check
