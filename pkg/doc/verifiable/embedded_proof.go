...
const (
	...
	psmsBlsSignatureProof2022   = "PsmsBlsSignatureProof2022"
)
...
func getProofType(proofMap map[string]interface{}) (string, error) {
	...
	case ed25519Signature2018, jsonWebSignature2020, ecdsaSecp256k1Signature2019,
		...
		psmsBlsSignature2022, psmsBlsSignatureProof2022:
	...
	for i := range proofs {
		...
		if len(opts.ldpSuites) == 0 {
			...
			case psmsBlsSignature2022:
				ldpSuites = append(ldpSuites, psmsblssignature2022.New(
					suite.WithVerifier(psmsblssignature2022.NewG1PublicKeyVerifier())))
			case psmsBlsSignatureProof2022:
				nonce, err := getNonce(proofs[i])
				if err != nil {
					return nil, err
				}
				ldpSuites = append(ldpSuites, psmsblssignatureproof2022.New(
