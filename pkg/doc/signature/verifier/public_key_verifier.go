...
// PSMSG1SignatureVerifier is a signature verifier that verifies a PSMS Signature
// taking Bls12381G1Key2022 public key bytes as input.
// Using public key bytes only, JWK is not supported for now
type PSMSG1SignatureVerifier struct {
	baseSignatureVerifier
}

// NewPSMSG1SignatureVerifier creates a new BBSG2SignatureVerifier.
func NewPSMSG1SignatureVerifier() *PSMSG1SignatureVerifier {
	return &PSMSG1SignatureVerifier{
		baseSignatureVerifier{
			keyType:   "EC",
			curve:     "BLS12381_G1",
			algorithm: "",
...
// Verify verifies the signature.
func (v *PSMSG1SignatureVerifier) Verify(pubKeyValue *PublicKey, doc, signature []byte) error {
	psms := psms12381g1pub.New()

	return psms.Verify(splitMessageIntoLines(string(doc), false), signature, pubKeyValue.Value) //TODO UMU (verifiable) Check doc processing depending on GetDigest result
}

// NewPSMSG1SignatureProofVerifier creates a new BBSG2SignatureProofVerifier.
func NewPSMSG1SignatureProofVerifier(nonce []byte) *PSMSG1SignatureProofVerifier {
	return &PSMSG1SignatureProofVerifier{
		nonce: nonce,
	}
}

// PSMSG1SignatureProofVerifier is a signature verifier that verifies a PSMS Signature Proof
// taking Bls12381G1Key2022 public key bytes as input.
// Supports public key bytes only, JWK is not supported for now.
type PSMSG1SignatureProofVerifier struct {
	baseSignatureVerifier
	nonce []byte
}

// Verify verifies the signature.
func (v *PSMSG1SignatureProofVerifier) Verify(pubKeyValue *PublicKey, doc, signature []byte) error { //TODO UMU (verifiable) Check doc processing depending on suite's behaviour
	psms := psms12381g1pub.New()

	return psms.VerifyProof(splitMessageIntoLines(string(doc), true),
		signature, v.nonce, pubKeyValue.Value)
}
