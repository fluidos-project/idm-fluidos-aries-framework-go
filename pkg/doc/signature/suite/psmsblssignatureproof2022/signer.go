...
const (
	...
	psmsBlsSignature2022 = "PsmsBlsSignature2022"
)
...
// SelectiveDisclosure creates selective disclosure from the input doc
func (s *Suite) SelectiveDisclosure(doc map[string]interface{}, revealDoc map[string]interface{},
	...
	blsSignatures, err := getPsmsProofs(rawProofs)
	if err != nil {
		return nil, fmt.Errorf("get BLS proofs: %w", err)
	}

	if len(blsSignatures) == 0 {
		return nil, errors.New("no PsmsBlsSignature2022 proof present")
	}
...
func generateSignatureProof(blsSignature map[string]interface{}, resolver keyResolver, nonce []byte,
	...
	signatureProofBytes, err := psms.DeriveProof(verData.blsMessages, signatureBytes,
		nonce, pubKeyBytes, verData.revealIndexes)
	if err != nil {
		return nil, fmt.Errorf("derive PSMS proof: %w", err)
	}
...
func getPublicKeyAndSignature(blsSignatureMap map[string]interface{}, resolver keyResolver) ([]byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("parse PSMS+ signature: %w", err)
	}

	keyID, err := blsSignature.PublicKeyID()
	if err != nil {
		return nil, nil, fmt.Errorf("get public KID from PSMS+ signature: %w", err)
	}

	publicKey, err := resolver.Resolve(keyID)
	if err != nil {
		return nil, nil, fmt.Errorf("resolve public key of PSMS+ signature: %w", err)
	}

	return publicKey.Value, blsSignature.ProofValue, nil
}

func getPsmsProofs(rawProofs interface{}) ([]map[string]interface{}, error) {
	allProofs, err := getProofs(rawProofs)
	if err != nil {
		return nil, fmt.Errorf("read document proofs: %w", err)
	}

	PsmsProofs := make([]map[string]interface{}, 0)

	for _, p := range allProofs {
		proofType, ok := p["type"].(string)
		if ok && strings.HasSuffix(proofType, psmsBlsSignature2022) {
			p["@context"] = securityContext
			PsmsProofs = append(PsmsProofs, p)
		}
	}

	return PsmsProofs, nil
}
...
func buildDocVerificationData(docCompacted, revealDoc map[string]interface{},
	...
	for i := range revealDocumentStatements {
		...
		statementInd := documentStatementsMap[statement] ///XXX UMU (issue/ZK) Does not check whether reveal statement was not in original statements (which admittedly should not happen)
...
func createVerifyDocumentData(doc map[string]interface{},
	...
	suite := psmsblssignature2022.New() //XXX UMU (verifiable/ZK) We use this to get the same document data that was signed in all canonicalization steps
	docBytes, err := suite.GetCanonicalDocument(doc, opts...)
	if err != nil {
		return nil, nil, fmt.Errorf("canonicalizing document failed: %w", err)
	}
	docDigest := suite.GetDigest(docBytes)
	documentStatements := splitMessageIntoLines(string(docDigest))
...
func createVerifyRevealData(doc map[string]interface{}, opts ...jsonld.ProcessorOpts) ([]string, error) {
	suite := psmsblssignature2022.New()
	docBytes, err := suite.GetCanonicalDocument(doc, opts...)
	if err != nil {
		return nil, fmt.Errorf("canonicalizing document failed: %w", err)
	}
	docDigest := suite.GetDigest(docBytes)
	return splitMessageIntoLines(string(docDigest)), nil
}
...
func createVerifyProofData(proofMap map[string]interface{}, opts ...jsonld.ProcessorOpts) ([]string, error) {
	...
	suite := psmsblssignature2022.New()
	docBytes, err := suite.GetCanonicalDocument(proofMapCopy, opts...)
	if err != nil {
		return nil, fmt.Errorf("canonicalizing document failed: %w", err)
	}
	docDigest := suite.GetDigest(docBytes)
