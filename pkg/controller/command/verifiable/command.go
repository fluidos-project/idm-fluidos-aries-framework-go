...
const (
	...
	//PsmsBlsSignature2022 signature suite
...
func (o *Command) DeriveCredential(rw io.Writer, req io.Reader) command.Error {
	...
	if hasPSMS(credential) {
		derived, err := credential.GeneratePSMSSelectiveDisclosure(request.Frame, []byte(request.Nonce),
