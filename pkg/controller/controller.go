...
import (
	...
	pocrest "github.com/hyperledger/aries-framework-go/pkg/controller/rest/poc"
...
func GetRESTHandlers(ctx *context.Provider, opts ...Opt) ([]rest.Handler, error) { // nolint: funlen,gocyclo
	...
	//PoC command
	pocOp, err := pocrest.New(vdrOp.GetVdrCommand(), wallet.GetWalletCommand())
	if err != nil {
		panic(err)
	}
	...
	allHandlers = append(allHandlers, pocOp.GetRESTHandlers()...)
...
func GetCommandHandlers(ctx *context.Provider, opts ...Opt) ([]command.Handler, error) { // nolint: funlen,gocyclo
	...
	//PoC command
	pocCmd, err := poc.New(vcmd, wallet)
	if err != nil {
		panic(err)
	}
	...
	allHandlers = append(allHandlers, pocCmd.GetHandlers()...)
