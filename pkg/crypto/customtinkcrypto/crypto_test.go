...
func TestPSMSCrypto_SignVerify_DeriveProofVerifyProof(t *testing.T) {
	c := Crypto{}
	msg := [][]byte{
		[]byte("<http://purl.org/dc/terms/created>\"2022-09-05T15:20:46.2270867+02:00\"^^<http://www.w3.org/2001/XMLSchema#dateTime> ."),
		[]byte("http://schema.org/name \"Jayden Doe\"^^<http://www.w3.org/1999/02/22-rdf-syntax-ns#HTML> ."),
		[]byte("http://schema.org/spouse \"did:example:c276e12ec21ebfeb1f712ebc6f1\" ."),
		[]byte("https://example.org/examples#degree http://www.w3.org/1999/02/22-rdf-syntax-ns#type <https://example.org/examples#BachelorDegree>. ."),
	}
	...
	var (
		s     []byte
		pubKH *psms.MockHandle
		badKH *psms.MockHandle
	)

	t.Run("test with PSMS signature", func(t *testing.T) {
		kt, err := psms.BLS12381G1KeyTemplate(kms.WithAttrs([]string{strconv.Itoa(nattr)}))
		require.NoError(t, err)
		kh, err := psms.NewMockHandle(kt)
		require.NoError(t, err)

		badKH, err = psms.NewMockHandle(tinkaead.KMSEnvelopeAEADKeyTemplate("babdUrl", nil))
		require.Errorf(t, err, "called mock handle with wrong keytemplate %s", "formatted")

		s, err = c.SignMulti(msg, kh)
		//fmt.Println(base58.Encode(kh.PrivateKey))
		//fmt.Println(base58.Encode(kh.PublicKey))
		//fmt.Println(fingerprint.CreateDIDKeyByCode(0xea, kh.PublicKey))
		require.NoError(t, err)
		...
		// sign with empty messages - should fail
		_, err = c.SignMulti([][]byte{}, kh)
		require.EqualError(t, err, "PSMS sign msg: messages are not defined")
		...
		pubKH, err = kh.Public()
		require.NoError(t, err)
		require.True(t, reflect.DeepEqual(kh.PublicKey, pubKH.PublicKey))
		err = c.VerifyMulti(msg, s, nil)
		require.EqualError(t, err, errBadKeyHandleFormat.Error())
		...
		err = c.VerifyMulti([][]byte{msg[0]}, s, pubKH)
		require.EqualError(t, err, "PSMS verify msg: messages are not defined")

		err = c.VerifyMulti([][]byte{msg[0], msg[1]}, s, pubKH)
		require.EqualError(t, err, "PSMS verify msg: mismatched number of attributes for pk:3,messages:1")

		err = c.VerifyMulti([][]byte{msg[0], msg[1], msg[2], msg[3][1:]}, s, pubKH)
		require.EqualError(t, err, "PSMS verify msg: invalid PSMS BLS12-381 signature")
	...
	t.Run("test with PSMS proof", func(t *testing.T) {
		revealedIndexes := []int{0, 2}
		nonce := make([]byte, 32)
		...
		err = c.VerifyProof([][]byte{msg[2], msg[3]}, proof, nonce, pubKH)
		require.EqualError(t, err, "verify proof msg: invalid PSMS BLS12-381 zk proof")

		err = c.VerifyProof([][]byte{msg[0], msg[2]}, proof, nonce[1:], pubKH)
		require.EqualError(t, err, "verify proof msg: invalid PSMS BLS12-381 zk proof")
