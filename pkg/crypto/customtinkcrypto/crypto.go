...
// Package customtinkcrypto provides a wrapper to the default implementation for adding psms with some mocks for integration. Note that
// it relies on primitives from default tinkcrypto pacakage
...
// Package customtinkcrypto includes the default implementation of pkg/crypto. It uses Tink for executing crypto primitives
...
func (t *Crypto) Sign(msg []byte, kh interface{}) ([]byte, error) {
	if _, ok := kh.(*psms.MockHandle); ok {
		return nil, fmt.Errorf("single message not supported for psms")
	}
	keyHandle, ok := kh.(*keyset.Handle)
	if !ok {
		return nil, errBadKeyHandleFormat
...
func (t *Crypto) Verify(sig, msg []byte, kh interface{}) error {
	if _, ok := kh.(*psms.MockHandle); ok {
		return fmt.Errorf("single message not supported for psms")
	}
	keyHandle, ok := kh.(*keyset.Handle)
	if !ok {
...
// SignMulti will create a PSMS/BBS+ signature of messages using the signer's private key in signerKH handle.
// returns:
// 		signature in []byte
...
func (t *Crypto) SignMulti(messages [][]byte, signerKH interface{}) ([]byte, error) {
	...
	if mockHandle, ok := signerKH.(*psms.MockHandle); ok {
		signer, err := psms.NewSignerMock(mockHandle)
		if err != nil {
			return nil, err
		}
		s, err := signer.Sign(messages)
		if err != nil {
			return nil, fmt.Errorf("PSMS sign msg: %w", err)
		}
		return s, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("create new BBS+ CUSTOM signer: %w", err)
	}
...
// VerifyMulti will  verify a signature of messages against the signer's public key in signerPubKH handle.
// returns:
// 		error in case of errors or nil if signature verification was successful
func (t *Crypto) VerifyMulti(messages [][]byte, signature []byte, signerPubKH interface{}) error {
	if mockHandle, ok := signerPubKH.(*psms.MockHandle); ok {
		verifier, err := psms.NewVerifierMock(mockHandle)
		if err != nil {
			return err
		}
		err = verifier.Verify(messages, signature)
		if err != nil {
			return fmt.Errorf("PSMS verify msg: %w", err)
		}
		return nil
	}
...
func (t *Crypto) VerifyProof(revealedMessages [][]byte, proof, nonce []byte, signerPubKH interface{}) error {
	if mockHandle, ok := signerPubKH.(*psms.MockHandle); ok {
		...
		if err != nil {
			return fmt.Errorf("verify proof msg: %w", err)
		}
		return nil
	}
...
//		error in case of errors
func (t *Crypto) DeriveProof(messages [][]byte, signature, nonce []byte, revealedIndexes []int,
	...
	if mockHandle, ok := signerPubKH.(*psms.MockHandle); ok {
		...
		if err != nil {
			return nil, fmt.Errorf("PSMS derive proof msg: %w", err)
		}
		return proof, nil
	}
