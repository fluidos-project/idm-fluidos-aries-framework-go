...
*/
// Package psms provides implementations of PSMS+ key management and primitives.
//
// The functionality of PSMS+ signatures/proofs is represented as a pair of
...
//
//      "github.com/hyperledger/aries-framework-go/pkg/crypto/tinkcrypto/primitive/psms"
//  )
//
//  func main() {
//      // create signer keyset handle
//      kh, err := keyset.NewHandle(psms.BLS12381G2KeyTemplate())
//      if err != nil {
//          //handle error
//      }
//
...
//      }
//
//		// finally get the PSMS+ signing primitive from the private key handle created above
//		s:= psms.NewSigner(kh)
//
...
//      }
//
//		// to verify, get the PSMS+ verification primitive from the public key handle created earlier above
//      v := psms.NewVerifier(verKH)
//
...
//      }
//
//		// to derive a proof from the psms signature, create the indices of the messages to be revealed by the proof
...
func init() {
	...
	err := registry.RegisterKeyManager(newPSMSSignerKeyManager())
	if err != nil {
		panic(fmt.Sprintf("psms.init() failed: %v", err))
	}

	err = registry.RegisterKeyManager(newPSMSVerifierKeyManager())
	if err != nil {
