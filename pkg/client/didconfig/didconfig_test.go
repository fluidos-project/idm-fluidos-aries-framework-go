...
func TestVerifyDIDAndDomain(t *testing.T) {
	...
	t.Run("error - http client error", func(t *testing.T) {
		...
		require.Error(t, err)
		matcher := regexp.MustCompile(`Get "https://non-existent-abc.com/.well-known/did-configuration.json": dial tcp: lookup non-existent-abc.com on ((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}: no such host`)
		require.NotNil(t, matcher.FindString(err.Error()))
