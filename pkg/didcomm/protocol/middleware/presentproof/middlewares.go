...
const (
	...
	psmsContext        = "https://ssiproject.inf.um.es/security/psms/v1"
)
...
func newPSMSSigner(km kms.KeyManager, cr crypto.Crypto, keyID string) *psmsSigner {
	return &psmsSigner{km: km, cr: cr, keyID: keyID}
}
...
// AddPSMSProofFn add PSMS proof to the Presentation.
func AddPSMSProofFn(p Provider) func(presentation *verifiable.Presentation) error { //XXX UMU ZK This method seems for signing presentations, not adding derived credentials to presentation, so it would not be part of usual flow
	km, cr := p.KMS(), p.Crypto()
	documentLoader := p.JSONLDDocumentLoader()

	return func(presentation *verifiable.Presentation) error {
		kid, pubKey, err := km.CreateAndExportPubKeyBytes(kms.BLS12381G1Type, kms.WithAttrs([]string{strconv.Itoa(nattrFromPresentation(presentation))}))
		if err != nil {
			return err
		}

		_, didKey := fingerprint.CreateDIDKeyByCode(fingerprint.BLS12381g1g2PubKeyMultiCodec, pubKey)

		presentation.Context = append(presentation.Context, psmsContext)

		return presentation.AddLinkedDataProof(&verifiable.LinkedDataProofContext{
			SignatureType:           "PsmsBlsSignature2022",
			SignatureRepresentation: verifiable.SignatureProofValue,
			Suite:                   psmsblssignature2022.New(suite.WithSigner(newPSMSSigner(km, cr, kid))),
			VerificationMethod:      didKey,
		}, jsonld.WithDocumentLoader(documentLoader))
	}
}

func nattrFromPresentation(presentation *verifiable.Presentation) int {
	//XXX UMU This would probably depend on what we decide for canon/digest..., for now just number of attributes in subjects (which might be the best/most stable,
	// Also look at another way of parsing credentials?
	var res int
	for _, cred := range presentation.Credentials() {
		res += nattrFromCredential(cred)
	}
	return res
}

func nattrFromCredential(cred interface{}) int {
	parsed, ok := cred.(*verifiable.Credential)
	if !ok {
		return 0
	}
	parsedSubjects := parsed.Subject.([]verifiable.Subject)
	var res int
	for _, s := range parsedSubjects {
		res += len(s.CustomFields)
	}
	return res
}
